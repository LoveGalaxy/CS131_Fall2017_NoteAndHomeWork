...
def conv(image, kernel):
    ...
    for h in range(Hi):
        for w in range(Wi):
            out[h, w] = np.sum(padded[h:h+Hk, w:w+Wk] * kernel)
    ### END YOUR CODE
    return out
...
def non_maximum_suppression(G, theta):
    ...
    for i in range(1,H-1):
        for j in range(1,W-1):
            # 如果直接判断角度来选择应该判断的坐标，会是代码比较冗余，考虑利用sin(theta)和cos(theta)
            alpha = np.deg2rad(theta[i, j])
            t1 = G[i-int(np.round(np.sin(alpha))), j-int(np.round(np.cos(alpha)))]
            t2 = G[i+int(np.round(np.sin(alpha))), j+int(np.round(np.cos(alpha)))]
            if not (G[i, j] >= t1 and G[i, j] >= t2):
                out[i, j] = 0
            else:
                out[i, j] = G[i, j]
    ### END YOUR CODE
